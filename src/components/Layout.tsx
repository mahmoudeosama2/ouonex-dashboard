...
const PRODUCT_ICONS: Record<string, ReactNode> = {
  ...
  cv_maker: <FileText className="w-3.5 h-3.5" />,
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center gap-2.5 px-5 h-16 border-b border-ink-800 shrink-0">
        ...
        <div>
          ...
          <p className="text-2xs text-ink-400 leading-tight">{t('topbar.connected')}</p>
        </div>
      </div>
        ...
        {visible.map(item => {
          ...
          return (
            ...
            >
              ...
              <span className="flex-1 text-start">{t('nav.' + item.key, { en: item.label, ar: item.label } as any)}</span>
      ...
      <div className="px-3 py-3 border-t border-ink-800 space-y-1.5">
        <div className="flex items-center gap-2 px-3 py-1.5">
          ...
          <span className="text-2xs text-ink-400">{t('topbar.all_operational')}</span>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="h-16 border-b border-ink-800 bg-ink-900/60 backdrop-blur-md sticky top-0 z-40 flex items-center justify-between px-4 lg:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={toggle}
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg border border-ink-800 hover:border-brand-500/40 bg-ink-950/60 hover:bg-ink-800/60 text-xs font-semibold text-ink-200 hover:text-ink-50 transition-all shadow-soft"
  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-4 mb-6">
      <div className="flex items-start gap-3">
        {icon && <div className="w-10 h-10 rounded-xl bg-ink-800 flex items-center justify-center text-ink-300 shrink-0">{icon}</div>}
